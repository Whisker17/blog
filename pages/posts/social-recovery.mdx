...
![](/images/blog/social-recovery-1.png)
...
```rust
let bounded*friends: FriendsOf<T> = friends.try_into().map_err(|*| Error::<T>::MaxFriends)?;
```
...
![](/images/blog/social-recovery-2.png)
...
- We need to retrieve the social recovery configuration for the lost account from the `recover_config`, if it is not there an error will be reported
...
pub fn claim*recovery(
    ...
    frame_system::Pallet::<T>::inc_consumers(&who).map_err(|*| Error::<T>::BadState)?;
...
- deposit a `RecoveryClosed` event
