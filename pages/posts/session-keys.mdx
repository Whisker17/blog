---
slug: session-keys
title: How to make our wallets more user-friendly
date: 2023-04-17
description: The article explains Substrate's Session Keys functionality, which improves wallet user experience and security by allowing users to set temporary hot keys for specific operations, eliminating the need for frequent password input.
...
- `set_keys`, allows an account to set its session key before becoming a validator, which is not valid until the next session
...
- `upgrade_keys`, update the type of keys, that need to be used with care, only in the `on_runtime_upgrade` block.
